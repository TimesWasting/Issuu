...
function requestCovers(folderItem) {
  arg = {
    ...
    access: "public",
    format: format,
    pageSize: 30, //default is 10; 30 is max to return (how many items to return)
...
function getData(URIlink, folderId) {
  var folderIdNew, coverSet, i, centerer, hoverer, count, divID, divFolder, name, itemsData, nameData, createdData, usernameData, documentIdData, items, itemCount;
...
function getPublications(start, pageQty, pageNum) {
  ...
  $.get(URIlink).done(function (data) {
    ...
    console.log(data);
    var pagesDiv, totalCount, divID, row, tagIf, tagList, tags, category, categoryData, username, documentIdData, documentId, folderString, folderId, pageCountData, pubDate1, pubDate2, pubDate3, pubDate4, pubDate5, pubDate6;
    ...
    for (i = 0; i < itemsListed; i++) {
      ...
      tagList = data.rsp._content.result._content[i].document.tags;
      ...
      */
      ...
      listingBlockEl.setAttribute('id', 'stream-list');
      divOutWrap = document.createElement('div');
